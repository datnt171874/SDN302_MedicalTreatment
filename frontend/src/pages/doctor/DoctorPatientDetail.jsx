...
const DoctorPatientDetail = () => {
  ...
  const displayAddress = patient.isAnonymous ? 'Không hiển thị' : patient.address;

  return (
    <Box sx={{ minHeight: '100vh', bgcolor: '#f5f5f5', py: 6, display: 'flex', justifyContent: 'center' }}>
        ...
        <Paper elevation={4} sx={{ p: 4 }}>
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={6}>
              <Typography><strong>Họ tên:</strong> {displayName}</Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography><strong>Email:</strong> {displayEmail}</Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography><strong>Mã bệnh nhân:</strong> {patient.code}</Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography><strong>Giới tính:</strong> {patient.gender}</Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography><strong>Ngày sinh:</strong> {patient.dob}</Typography>
            </Grid>
            <Grid item xs={12} sm={6}>
              <Typography><strong>Số điện thoại:</strong> {displayPhone}</Typography>
            </Grid>
            <Grid item xs={12}>
              <Typography><strong>Địa chỉ:</strong> {displayAddress}</Typography>
            </Grid>
          </Grid>
